...
shares <- readxl::read_excel("All_data.xlsx", sheet = "Shares")
liabilities <- readxl::read_excel("All_data.xlsx", sheet = "Assets")
market_caps <- readxl::read_excel("All_data.xlsx", sheet = "Capitalizations")
betaMV <- betaMV %>% select(-Date)*(market_caps[2:nrow(market_caps),2:ncol(market_caps)])

#Selecting institutions and variables
...
#☺Calculating returns from shares
...
#Creating rankings from variables
...
#Calculating PCA for daily rankings
...
#Calculating PCA for monthly rankings
...
#Calculating the average downturn under a certain threshold

threshold <- -0.025

change_returns <- function(x, na.rm=FALSE) (ifelse(x <= threshold, x, NA))
...
#Calculating PLS for monthly variables, regressing target variable without lag order
...
#Create rankings from PCA scores

pca_scores <- bind_rows(scores_firstcomp_month, scores_secondcomp_month, .id = 'Component') %>% 
  relocate(c("Component", "Date", inst_analysed)) %>%
  rename(Variable = Component) %>% mutate(Variable = paste("PCA", Variable, sep = "_"))
...
#Create rankings from PLS scores
...
#Calculate average root-mean-squared deviations for institutions
...
#Calculate deviation for locations in each subperiod
...
#Create rankings from deviations from PCA
...
#Create rankings from deviations from PLS
...
#Spearman correlation between monthly rankings
...
#Calculate quantiles of correlation time series
...
#Caclulate rankings' volatility in subperiods
...
#Calculate how many institutions remained in the top ten from previous years' certain month

#Here, the month is August (month == 8)
...
#Calculating regressions for each subperiod
#lag_order remains 1, meaning that it is regressing the one month forward returns
...
#Plotting Variable for institutions
...
#Plotting initial spillover from measure
...
#Plotting Spearman correlation at a given time between initial variables
...
#Plotting daily explained variances by PCA
...
#Plotting monthly explained variances by PCA
...
#Plotting explained variances by PLS
...
#Plotting deviations and negative returns
